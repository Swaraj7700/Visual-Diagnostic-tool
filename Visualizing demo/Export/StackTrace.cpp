...
void StackTrace::printStackTrace(HANDLE hProcess)
{
    ...
    bool sym = SymInitialize(hProcess, "C:\\Users\\swara\\source\\repos\\edisem3\\Debug", TRUE);
            ...
            if (hThread)
            {
                ...
                if (GetThreadContext(hThread, &context))
                {
                        ...
                        SymFunctionTableAccess64, SymGetModuleBase64, nullptr))
                    {
                        ...
                        {
                            //frame->ToString();
                            strFunctionNames.emplace_back(frame->GetFunctionName());
                            //+"@" + frame->GetParameterName() + "&" + frame->GetVariableName() + "$");
