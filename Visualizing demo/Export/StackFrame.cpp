...
std::string StackFrame::GetFunctionName() {
...
std::string StackFrame::GetParameterName() {
...
std::string StackFrame::GetVariableName() {
	...
	for (UINT i = 0; i < 3; ++i)
	{
		if (i == 0)
		{
			for (UINT j = 0; j < localVariables.size(); ++j)
			{
				flagudt = localVariables[j].isUdt();
				flagarr = localVariables[j].isArr();
				if (!flagudt && !flagarr)
				{
					if (j > 0)
						ret << " ";
					ret << localVariables[j].ToString();
				}
			}
		}
